...
function factory (db, options) {
  var LevelUP = db.constructor
  ...
  function Transaction (db, options) {
    ...
    if (db instanceof Transaction) {
      ...
      // db is sublevelup, get its levelupcreate
...
module.exports = function main (db, options) {
  if (!Transaction) Transaction = factory(db, options)
